...
describe('EssayQuoteModal — writing', () => {
	...
	it('writes a book-linked quote, registers it, and sets it in', async () => {
		...
		(m.findComponent({ name: 'SourceSelector' }).vm as unknown as { report: (r: object) => void }).report({
			mode: 'book',
			bookId: 'bge',
			book: BGE,
			page: '89',
	...
	it('links an author with a connection, and credits them by name', async () => {
		...
		await m.find('textarea').setValue(WRITTEN.quote);
		(m.findComponent({ name: 'SourceSelector' }).vm as unknown as { report: (r: object) => void }).report({ mode: 'author', authorId: 'newton' });
		await flushPromises();
