...
<template>
    <div class="relative w-full h-full flex items-center justify-center overflow-y-auto overscroll-contain py-6">
        <QuoteSlideBody :text="text" :creator="creator || undefined" :work="work || undefined" :year="year || undefined" :page="page || undefined" :font-size="effectiveFontSize" :justified="justified" :hyphenation="hyphenation" :typography-class="typographyClass" with-quotation-marks :mode="mode ?? 'textured'" :texture-url="textureUrl" :fallback-texture-url="fallbackTextureUrl" :darkness="darkness" />
    </div>
</template>
