  ...
  <div class="w-full max-w-2xl xl:max-w-3xl mx-auto px-4 mt-4 sm:mt-6 pb-20">
    ...
    <div v-if="mode === 'shuffle'" key="shuffle">
      ...
      <div class="mt-4 sm:mt-5 space-y-4 sm:space-y-5">
        ...
        <template v-else>
          <NoteCard
            v-for="(note, i) in dealNotes"
            :key="`${dealKey}-${note.id}`"
            :note="note"
            :isAdmin="isAdmin"
            :book="bookFor(note)"
            :connectedAuthor="noteAuthors.get(note.id) ?? null"
            :latestThread="noteThreads.get(note.id) ?? null"
            variant="deal"
            class="deal-in"
            ...
            @addToThread="emit('addToThread', $event)"
      ...
      <div class="flex items-center gap-2 py-2">
        ...
        <div class="flex-1 relative">
          <input
            ...
            class="w-full pl-9 pr-3 py-2 bg-mono-900 border border-mono-800 rounded-lg focus:outline-none focus:border-accent transition-colors placeholder:text-mono-600 placeholder:italic text-sm text-white"
          />
          ...
          <div class="overflow-hidden min-h-0">
            <div class="border-t border-mono-800">
              ...
              <div class="max-h-72 overflow-y-auto overscroll-contain">
                <button
                  v-for="{ book, count } in menuBooks"
                  :key="book.id"
                  @click="selectBook(book.id)"
                  class="w-full flex items-baseline gap-2 px-4 py-2.5 hover:bg-mono-800 active:bg-mono-800 transition-colors text-left cursor-pointer border-b border-mono-800/50"
                >
