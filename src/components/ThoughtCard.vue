...
<template>
	<div class="group relative bg-mono-900 border border-rose/20 hover:border-rose/40 rounded-xl p-4 transition-all duration-200 hover:shadow-lg hover:shadow-rose/10 cursor-pointer" @click="emit('present', thought)">
		<!-- Content -->
		<p lang="en" class="typography-prose text-mono-100 text-sm leading-relaxed whitespace-pre-wrap wrap-break-word" v-html="formattedContent"></p>
		...
		<div class="flex items-center justify-between mt-3 pt-3 border-t border-mono-800">
			...
			<div class="flex items-center gap-1 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
				...
				<button v-if="props.isAdmin" @click.stop="emit('edit', thought)" class="p-1.5 text-mono-500 hover:text-rose-bright transition-colors rounded-lg hover:bg-rose/10 cursor-pointer" title="Edit">
					<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
				...
				<button v-if="props.isAdmin" @click.stop="emit('delete', thought.id)" class="p-1.5 text-mono-500 hover:text-red-400 transition-colors rounded-lg hover:bg-red-500/10 cursor-pointer" title="Delete">
					<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
						<path d="M3 6h18" />
