...
**Drafts are kept in `localStorage`, unencrypted.** `useDraft.ts` persists an
unsent note in the browser so a reload does not lose it, and `LibraryPage`
remembers a sort order the same way. On a shared machine, a draft outlives the
Access session that wrote it. Nothing clears it on sign-out, because there is no
sign-out here: the session is Cloudflare's.
